...
* v2.4.2：Windows 顶行三处修正：系统窗口按钮改用行图标的灰（原来用正文墨色，比谁都黑）、菜单按钮挪到整行最左端、左边右边间距对称
* v2.4.1：全屏不再给已经不存在的红绿灯留位置；Windows 的外壳从三条横杆变成一行，菜单成为行尾的第四个图标；焦点框跟主题走而不是系统强调色；窗口菜单说界面语言；新建标签就能直接打字
* v2.4.0：顶行重做：左边只有文档，右边是面板和打开它的三个按钮。标签占满整行、宽度有了上下限，外壳所有细线统一成一个不透明的灰，加号不会再被标签挤走，切换面板改用 `⌘\`

* v2.2.0：顶部永远只有一行，标签栏常驻，一个文档也是一个标签；再打开文件落成标签页而不是新窗口；窗口记住大小与缩放；PDF 导出有了跟随主题底色的页边距；新增「格式」子菜单与脚注悬停预览；界面颜色全面跟随主题
...
* v2.1.0：标签页，一个窗口里同时开多个文档，每个标签各自保有内容、未保存状态、撤销历史与滚动位置；`⌘T`、File 菜单或文件列表里 ⌘ 加点击都能开一个；只有一个文档时不出现标签栏

* v2.0.7：macOS 按芯片分别发布，首次下载从 216MB 降到约 82MB；新增 Windows 免安装 zip 版；文件面板支持右键菜单；文件名保持居中

* v2.0.6：标题栏修正，「打开所在文件夹」按钮恢复可用、提示标签不再被正文压住或半透明、右上角按钮与上边距对齐
