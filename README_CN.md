...
- **关闭项持续隐藏** — 已关闭的文件在当前窗口会话内不会因打开其他文件而重新出现在左侧；通过“打开”主动载入即可恢复显示。

- **关闭列表项** — 关闭当前文档时，该文件名会从左侧列表移除；其他同目录文件保留。

- **关闭或删除文件** — 左侧当前文件的右键菜单支持仅关闭列表项，也支持确认后永久删除磁盘文件。

- **可靠的文档切换** — 使用应用内“保存并切换 / 不保存 / 取消”确认界面，切换完成后可直接继续输入，不再丢失键盘焦点。

- **保留文件列表** — 关闭当前文档后，同目录文件继续留在左侧，可直接点击打开另一篇文档。

- **空白新文档** — 关闭当前文档或新建时使用真正的空白编辑页，并自动将光标置入编辑区。
